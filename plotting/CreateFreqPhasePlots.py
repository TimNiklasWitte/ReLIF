...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(relif_1_resonating)

        axes[0].scatter(relif_1_freq, relif_1_phase, c=relif_1_resonating)

        axes[0].set_xlim(0, model.relif_1.freq_max)
        axes[0].set_ylim(0, 2 * torch.pi)
        axes[0].set_xlabel('Frequency', fontsize=12)
        axes[0].set_ylabel('Phase', fontsize=12)
        axes[0].set_title('Hidden Layer', fontsize=14)
        ...
        axes[1].scatter(relif_2_freq, relif_2_phase, c=relif_2_resonating)

        axes[1].set_xlim(0, model.relif_2.freq_max)
        axes[1].set_ylim(0, 2 * torch.pi)
        axes[1].set_xlabel('Frequency', fontsize=12)
        axes[1].set_ylabel('Phase', fontsize=12)
        axes[1].set_title('Output Layer', fontsize=14)
        axes[1].grid(True, alpha=0.3)

        

        #plt.scatter(relif_1_freq, relif_1_phase)

        plt.suptitle(f"Epoch: {epoch}", fontsize=16)
        plt.tight_layout()
        plt.savefig(f"./plots/Epoch_{epoch}.png", dpi=200)
        plt.close()
